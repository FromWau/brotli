...
private fun unpackCommandLookupTable(cmdLookup: ShortArray): Unit {
  ...
  for (cmdCode: Int in 0 until 704) {
    ...
    val distanceContext: Int = distanceContextOffset + min(copyLengthOffset, 5) - 2;
...
private fun maybeReallocateRingBuffer(s: State): Unit {
  ...
  if (oldBuffer.size != 0) {
    oldBuffer.copyInto(destination = newBuffer, destinationOffset = 0, startIndex = 0, endIndex = s.ringBufferSize);
  }
...
private fun readMetablockHuffmanCodesAndContextMaps(s: State): Int {
  ...
  while (i < s.numLiteralBlockTypes) {
    val limit: Int = min(i + 96, s.numLiteralBlockTypes);
    while (i < limit) {
      fillBitWindow(s);
...
private fun copyUncompressedData(s: State): Int {
  ...
  val chunkLength: Int = min(s.ringBufferSize - s.pos, s.metaBlockLength);
...
private fun writeRingBuffer(s: State): Int {
  val toWrite: Int = min(s.outputLength - s.outputUsed, s.ringBufferBytesReady - s.ringBufferBytesWritten);
  if (toWrite != 0) {
    s.ringBuffer.copyInto(destination = s.output, destinationOffset = s.outputOffset + s.outputUsed, startIndex = s.ringBufferBytesWritten, endIndex = s.ringBufferBytesWritten + toWrite);
...
private fun doUseDictionary(s: State, fence: Int): Int {
  ...
  if (address < 0) {
    ...
    val dictionaryData: ByteArray = data;
...
private fun copyFromCompoundDictionary(s: State, fence: Int): Int {
  ...
  while (s.cdBrLength != s.cdBrCopied) {
    ...
    s.cdChunks[s.cdBrIndex]!!.copyInto(destination = s.ringBuffer, destinationOffset = pos, startIndex = s.cdBrOffset, endIndex = s.cdBrOffset + length);
    pos += length;
    s.cdBrOffset += length;
    s.cdBrCopied += length;
    if (length == remChunkLength) {
...
internal fun transformDictionaryWord(dst: ByteArray, dstOffset: Int, src: ByteArray, srcOffset: Int, wordLen: Int, transforms: Transforms, transformIndex: Int): Int {
...
internal fun copyRawBytes(s: State, data: ByteArray, offset: Int, length: Int): Int {
  ...
  val copyNibbles: Int = min(halfAvailable(s), len shr 2);
  if (copyNibbles > 0) {
    val readOffset: Int = s.halfOffset shl 2;
    val delta: Int = copyNibbles shl 2;
    s.byteBuffer.copyInto(destination = data, destinationOffset = pos, startIndex = readOffset, endIndex = readOffset + delta);
    pos += delta;
    len -= delta;
...
fun setData(newData: ByteArray, newSizeBits: IntArray): Unit {
...
private fun unpackDictionaryData(dictionary: ByteArray, data0: String, data1: String, skipFlip: String, sizeBits: IntArray, sizeBitsData: String): Unit {
  ...
  dict.copyInto(dictionary);
}

private fun dictionaryDataStaticInit(): Unit {
  val dictionaryData: ByteArray = ByteArray(122784);
...
internal fun fillBytesWithZeroes(dest: ByteArray, start: Int, end: Int): Unit {
  ...
  while (cursor < end) {
    var step: Int = min(cursor + 1024, end) - cursor;
    BYTE_ZEROES.copyInto(destination = dest, destinationOffset = cursor, startIndex = 0, endIndex = step);
    cursor += step;
  }
}
...
internal fun fillIntsWithZeroes(dest: IntArray, start: Int, end: Int): Unit {
  ...
  while (cursor < end) {
    var step: Int = min(cursor + 1024, end) - cursor;
    INT_ZEROES.copyInto(destination = dest, destinationOffset = cursor, startIndex = 0, endIndex = step);
    cursor += step;
  }
}

internal fun copyBytesWithin(bytes: ByteArray, target: Int, start: Int, end: Int): Unit {
  bytes.copyInto(destination = bytes, destinationOffset = target, startIndex = start, endIndex = end);
}

internal fun readInput(s: State, dst: ByteArray, offset: Int, length: Int): Int {
  val source: Source = s.input ?: return -1;
  try {
    return source.readAtMostTo(dst, offset, offset + length);
  } catch (e: IOException) {
